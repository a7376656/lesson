...
abstract class Model implements \JsonSerializable, \ArrayAccess
{
    ...
    public function __construct($data = [])
    {
        if (strpos($this->table, '.') !== false) {
            ...
            $this->connection = Config::get('database.'.$tableArr[0]);
            $this->table = $tableArr[1];
        }
     ...
     * @access public
     * @param array  $data     数据
     * @param array|bool  $where    更新条件|是否保存:是-true，否-false
    ...
    public function save($data = [], $where = [], $sequence = null)
    {
        ...
        if (!empty($data)) {
            ...
            //传递过来的数组中含有主键并且不进行保存操作则进行更新
            if ((array_key_exists($pk, $data) && true !== $where) || (is_array($where) && !empty($where))) {
                $this->isUpdate = true;
            }
        }
